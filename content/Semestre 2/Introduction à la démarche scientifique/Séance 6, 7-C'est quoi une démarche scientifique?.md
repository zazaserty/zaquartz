...
La démarche scientifique est une approche de connaissance. Elle implique une réflexivité du chercheur et une rupture avec les préjugés (faut être objectifs) et le savoir commun (implicitement considérés comme vrais ou valides par défaut).
*Le fait scientifique est conquis (sur les préjugés->nécessité de les dépasser, les faits n'apparaissent pas par magie), construit (par la raison) et constaté (par les faits)* (Gaston Bachelard).
#### Ainsi :
...
- La démarche scientifique implique une évaluation constante de ses propres concepts, lesquels doivent présenter : 
  - Clarté (correspondance entre le terme et la signification du concept).
  - Précision (doit correspondre le plus directement possible les aspects du réel auxquels il s'applique).
...
1. La théorie : un cadre explicatif validé
...
1. L'hypothèse : une supposition testable et plus spécifique
...
### Arguments
![[Pasted image 20250401142933.png]]
#### Exemple
![[Pasted image 20250401142945.png]]
#### Cohérence logique et arguments
![[Pasted image 20250401143007.png]]
![[Pasted image 20250401143040.png]]
![[Pasted image 20250401143245.png]]
La 4 est fausse puisqu'il peut y avoir des structures pour empêcher une rébellion, et la société pourrait ne pas être juste même en absence de rébellion. Dans le cas de la 5 sa première premise ne dit rien, c'est évident que les récidivistes récidivent. On pourrait dire qu'il faut les mettre dans un état où ils ne peuvent pas récidiver, mais là notre raisonnement est tautologique.
![[Pasted image 20250401143724.png]]
#### Donc
Il faut donc se méfier des arguments fallacieux ou des sophismes (arguments apparemment bien formés mais qui ne contiennent pas de vérité démontrée, ou alors faux), tels que:
- Appel aux normes majoritaires (tout le monde pense que A, donc…).
- Généralisations hâtives (les personnes que je côtoie pensent A, donc la majorité pense A).
- La pente glissante (si on fait A, alors B et donc C).
- Le lien causal douteux (s'il pleut, il fait humide; aujourd'hui il fait humide, donc il pleut).
- L'appel à la (fausse) autorité (il/elle sait, donc ce qui est dit est vrai).
- La fausse analogie…. un exemple….
![[Pasted image 20250401144218.png]]
### Conclusion
- Maintenant que nous avons quelques outils de base, la question qui va nous occuper est: comment pense un chercheur en sciences sociales ?
- Les questions de base à se poser: 
  - Qu’est-ce que le phénomène A ?
  - Comment peut-on connaitre le phénomène A ?
  - Qu'est ce qui détermine /explique le phénomène A ?
  - Comment peut-on appréhender (mesurer, comprendre, interpréter, etc.) le phénomène A ?
  - (Quel jugement peut-on porter sur le phénomène A ? C'est le propre de l'analyse normative / évaluative).
